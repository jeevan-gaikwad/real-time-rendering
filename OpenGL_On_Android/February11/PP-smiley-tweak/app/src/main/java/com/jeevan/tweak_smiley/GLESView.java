package com.jeevan.tweak_smiley;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private void initialize(GL10 gl){
		...
		//load texture images
		texture_smiley[0] = loadTextureFromImage(R.raw.smiley);
		GenerateGLTextures();
		...
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, 32, null, GLES30.GL_DYNAMIC_DRAW);
	...
	public void draw(){
		float smileyFaceTexcoords[]=new float[8];
		if (onTouchCount == 1) { //show 0.5
	...
	}else if (onTouchCount == 2) { //show 0.5
		smileyFaceTexcoords[0] = 0.0f;
		...
		ByteBuffer byteBuffer =  ByteBuffer.allocateDirect(smileyFaceTexcoords.length * 4); //4 is size of float. This is global mem allocation. All memory location will be initialized to zero
		byteBuffer.order(ByteOrder.nativeOrder()); //Detect native machine endianess and use it
		FloatBuffer verticesBuffer = byteBuffer.asFloatBuffer();
		...
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, vbo_texure[0]);
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, smileyFaceTexcoords.length * 4, verticesBuffer, GLES30.GL_DYNAMIC_DRAW);
		GLES30.glActiveTexture(GLES30.GL_TEXTURE0);
		if(onTouchCount == 0)
			GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, texture_board[0]);
		else
	...
	private int loadTextureFromImage(int imageFileResourceID){
	...
	void MakeColoredBoard() {
		...
		for (i = 0;i < checkImageHeight;i++) {
			for (j = 0;j < checkImageWidth;j++) {			
				coloredBoardTexture[i][j][0] = 255;
				coloredBoardTexture[i][j][1] = 255;
				coloredBoardTexture[i][j][2] = 255;
				coloredBoardTexture[i][j][3] = 1;
			}
		}
    }
	void GenerateGLTextures() {
		byte[] localTexture=new byte[16384];
		java.util.Arrays.fill(localTexture, (byte)255);
		MakeColoredBoard();
		GLES30.glPixelStorei(GLES30.GL_UNPACK_ALIGNMENT, 1);
		GLES30.glGenTextures(1, texture_board, 0);
		GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, texture_board[0]);
		GLES30.glTexParameteri(GLES30.GL_TEXTURE_2D, GLES30.GL_TEXTURE_WRAP_S, GLES30.GL_REPEAT);
		GLES30.glTexParameteri(GLES30.GL_TEXTURE_2D, GLES30.GL_TEXTURE_WRAP_T, GLES30.GL_REPEAT);
		GLES30.glTexParameteri(GLES30.GL_TEXTURE_2D, GLES30.GL_TEXTURE_MAG_FILTER, GLES30.GL_NEAREST);
		GLES30.glTexParameteri(GLES30.GL_TEXTURE_2D, GLES30.GL_TEXTURE_MIN_FILTER, GLES30.GL_NEAREST);
		ByteBuffer byteBuffer =  ByteBuffer.allocateDirect(localTexture.length); //4 is size of float. This is global mem allocation. All memory location will be initialized to zero
		byteBuffer.order(ByteOrder.nativeOrder()); //Detect native machine endianess and use it
		byteBuffer.put(localTexture);
