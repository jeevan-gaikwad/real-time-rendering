package com.jeevan.light_toggle_vertex_fragment;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private int		isPerVertexLightingUniform, isPerFragmentLightingUniform;
	private boolean isPerFragment, isPerVertex = true;
	...
	private void initialize(GL10 gl){
		...
		"uniform int u_is_per_vertex_lighting;" +
		"uniform vec3 u_La;" +
		"uniform vec3 u_Ld;" +
		"uniform vec3 u_Ls;" +
		"uniform vec3 u_Ka;" +
		...
		"void main(void)" +
		"{" +
		"int u_lighting_enabled =1; /* Keeping lights by default ON */" +
		...
		"uniform vec3 u_Ks;" +
		"uniform float u_material_shininess;" +
		"uniform int u_is_per_fragment_lighting;" +
		"out vec4 FragColor;" +
		"void main(void)" +
		"{" +
		"vec3 phong_ads_color_local;"+
		"int u_lighting_enabled=1; /* Keeping lights bydefault ON*/" +
		"if(u_lighting_enabled == 1)" +
		"{" +
		"if(u_is_per_fragment_lighting == 1)" +
		"{"+
		"vec3 normalized_transformed_normals = normalize(transformed_normals);" +
		"vec3 normalized_light_direction = normalize(light_direction);" +
		"vec3 normalized_viewer_vector = normalize(viewer_vector);" +
		...
		"FragColor = vec4(phong_ads_color_local, 1.0);" +
		"}" +
		"else" +
		"{" +
		...
		"else" +
		"{" +
		"phong_ads_color_local = vec3(1.0, 1.0, 1.0);" +
		"FragColor = vec4(phong_ads_color_local, 1.0);" +
		"}" +		
		"}"
		);
		...
		isPerVertexLightingUniform = GLES30.glGetUniformLocation(shaderProgramObject, "u_is_per_vertex_lighting"); //V/v key pressed or not
		isPerFragmentLightingUniform = GLES30.glGetUniformLocation(shaderProgramObject, "u_is_per_fragment_lighting"); //F/f key pressed or not
	...
	public void draw(){
		...
		GLES30.glUseProgram(shaderProgramObject);
			...
			if (isPerVertex) {
				GLES30.glUniform1i(isPerVertexLightingUniform, 1);
				GLES30.glUniform1i(isPerFragmentLightingUniform, 0);
			}
			else if(isPerFragment)
			{
				GLES30.glUniform1i(isPerFragmentLightingUniform, 1);
				GLES30.glUniform1i(isPerVertexLightingUniform, 0);
			}
