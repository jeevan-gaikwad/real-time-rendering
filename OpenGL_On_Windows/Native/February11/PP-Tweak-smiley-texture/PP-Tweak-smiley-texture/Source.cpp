...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("PP- Texture- smiley -tweak");
	bool bDone = false;
...
void initialize() {
	void resize(int, int);
	void uninitialize();
	int LoadTextureFromImage(GLuint *texture, TCHAR imageResourceId[]);
	...
	glBufferData(GL_ARRAY_BUFFER, 32, NULL, GL_STATIC_DRAW); //we'll provide texture coordinates
	...
	//Texture
	LoadTextureFromImage(&gTexture_smiley_face, MAKEINTRESOURCE(IDB_BITMAP_SMILEY_FACE));
