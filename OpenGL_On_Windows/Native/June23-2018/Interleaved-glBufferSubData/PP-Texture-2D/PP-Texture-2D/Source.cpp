...
//				g_log_file << "Shader program linking log:" << szInfoLog << std::endl;
	...
	//set square position
	//This is to demonstrate usage of glBufferSubData
	glGenBuffers(1, &gVbo_cube_buffer);
	glBindBuffer(GL_ARRAY_BUFFER, gVbo_cube_buffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(cubeVertices)+ sizeof(cubeNormals) +
		sizeof(cubeColors) + sizeof(cubeTexcoords), NULL, GL_STATIC_DRAW);
	...
	//Map buffer to attributes
	glVertexAttribPointer(JCG_ATTRIBUTE_VERTEX, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);
	glEnableVertexAttribArray(JCG_ATTRIBUTE_VERTEX);
	
	glVertexAttribPointer(JCG_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, (GLvoid*)sizeof(cubeVertices));
	glEnableVertexAttribArray(JCG_ATTRIBUTE_NORMAL);

	glVertexAttribPointer(JCG_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, (GLvoid*)(sizeof(cubeVertices) + sizeof(cubeNormals)));
	glEnableVertexAttribArray(JCG_ATTRIBUTE_COLOR);

	glVertexAttribPointer(JCG_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 0, (GLvoid*)(sizeof(cubeVertices)+sizeof(cubeNormals)+ sizeof(cubeColors)));
...
void uninitialize(void)
{
	...
	if (gVbo_cube_buffer) {
		glDeleteBuffers(1, &gVbo_cube_buffer);
		gVbo_cube_buffer = 0;
	}
