...
#define DEFAULT_AXIS_ROTATION 'x'
...
GLuint gModelLightMatrixUniform; //light0
GLuint gModelLight1MatrixUniform; //light1
GLuint gModelLight2MatrixUniform; //light1
...
void initialize_shaders() {
	...
	const GLchar *vertexShaderSourceCode =         //Source code of Vertex shader
		...
		"{" \
		"        /*light calculations  */       " \
		"vec4 eye_coordinates = u_view_matrix * u_model_light_matrix * vPosition;" \
		"transformed_normals = mat3(u_view_matrix * u_model_light_matrix) * vNormal;" \
...
void initialize() {
	...
	//light animation
	gModelLightMatrixUniform = glGetUniformLocation(gShaderProgramObject, "u_model_light_matrix");
...
void draw_spheres(GLint no_of_spheres, material_props_t material_properties[], int no_of_materials) {
	...
	modelMatrix = vmath::translate(x_trans, y_trans, -10.0f);
...
void set_light_rotation(char axis_of_rotation) {
	...
	if (axis_of_rotation == 'x') {
		rotationLightMatrix = vmath::rotate(gAngle, 1.0f, 0.0f, 0.0f);
	}
	else if (axis_of_rotation == 'y') {
		rotationLightMatrix = vmath::rotate(gAngle, 0.0f, 1.0f, 0.0f);
	}
	else if (axis_of_rotation == 'z') {

		rotationLightMatrix = vmath::rotate(gAngle, 0.0f, 0.0f, 1.0f);
		lightPosition[1] = 360.0f; //For z axis rotation visibility
		glUniform4fv(gLightPositionUniform, 1, lightPosition);
	}
	//set the light position
	modelLightMatrix = modelLightMatrix * rotationLightMatrix;
	glUniformMatrix4fv(gModelLightMatrixUniform, 1, GL_FALSE, modelLightMatrix); //light
}
