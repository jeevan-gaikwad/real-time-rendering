...
GLuint gIsPerVertexLightingUniform, gIsPerFragmentLightingUniform;
GLuint La_uniform, Ld_uniform, Ls_uniform;
GLuint Ka_uniform, Kd_uniform, Ks_uniform;
GLuint material_shininess_uniform;
GLfloat gAngle = 0.0f;
bool gbAnimate;
bool gbLight, gbIsPerVertex = true; //Turn per vertex lighting by default on
bool gbIsPerFragment = false;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("PP-Toggling-per-vertex-per-fragment-Phong");
	bool bDone = false;
...
void initialize() {
	...
	gIsPerVertexLightingUniform = glGetUniformLocation(gShaderProgramObject, "u_is_per_vertex_lighting"); //V/v key pressed or not
	gIsPerFragmentLightingUniform = glGetUniformLocation(gShaderProgramObject, "u_is_per_fragment_lighting"); //F/f key pressed or not
...
void display() {
	...
	if (gbLight == true) {
		...
		if (gbIsPerVertex) {
			glUniform1i(gIsPerVertexLightingUniform, 1);
			glUniform1i(gIsPerFragmentLightingUniform, 0);
		}
		else if(gbIsPerFragment)
		{
			glUniform1i(gIsPerFragmentLightingUniform, 1);
			glUniform1i(gIsPerVertexLightingUniform, 0);
		}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 0x76: //v
			if (gbIsPerVertex) {
				gbIsPerVertex = false;
				gbIsPerFragment = true;
			}
			else {
				gbIsPerVertex = true;
				gbIsPerFragment = false;
			}
			break;
		case 0x46: //F
		case 0x66: //f
