...
ID3D11Buffer *gpID3D11Buffer_QuadColorBuffer = NULL;
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		"float3 s=(float3)normalize(lightPos-eyeCoords);" \
	...
	const char *pixelShaderSourceCode =
		"float4 main(float4 pos: POSITION, float4 diffuseLight:COLOR) : SV_TARGET" \
		"{"\
		"return(diffuseLight);"\
		"}";
	...
	float cubeVertices[] =
	{
		...
		-1.0f, -1.0f, -1.0f, //left bottom of 2nd triangle
		1.0f, 1.0f, -1.0f, //right top of 2nd triangle
		1.0f, -1.0f, -1.0f, //right bottom of 2nd triangle
		//right side
		1.0f, 1.0f, -1.0f, //right top
		1.0f, 1.0f, 1.0f, //left top
		1.0f, -1.0f, -1.0f, //left bottom
	...
	bufferDescNormals.ByteWidth = sizeof(float) * ARRAYSIZE(cube_normals);
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceNormalsCube;
	ZeroMemory(&mappedSubresourceNormalsCube, sizeof(D3D11_MAPPED_SUBRESOURCE));

	gpID3D11DeviceContext->Map(gpID3D11Buffer_QuadColorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresourceNormalsCube);
	memcpy(mappedSubresourceNormalsCube.pData, cube_normals, sizeof(cube_normals));
...
void display()
{
	...
	constantBufferLightProp.ld = XMVectorSet( 1.0f, 1.0f, 1.0f,0.0f );
	constantBufferLightProp.kd = XMVectorSet(0.5f, 0.5f, 0.5f,0.0f );
	constantBufferLightProp.position = XMVectorSet( 0.0f, 0.0f, 2.0f, 1.0f);
