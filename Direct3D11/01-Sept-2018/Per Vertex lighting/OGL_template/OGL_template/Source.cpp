...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		"float4 lightDirection=(float4)normalize((float4)lightPos-eyeCoords);" \
		"float4 ambient = la * ka;" \
		"float4 diffuse = ld * kd * max(dot(lightDirection,float4(tNorm,0.0)),0.0);" \
		"float4 reflectionVector = reflect(-lightDirection,float4(tNorm,0.0));" \
		"float3 viewerVector = (float3)normalize(-eyeCoords.xyz);" \
		"float4 specular = ls * ks * pow(max(dot(reflectionVector,float4(viewerVector,0.0)),0.0), materialShininess);" \
