...
HRESULT initialize()
{
	...
	gClearColor[0] = 0.25f; //R
	gClearColor[1] = 0.25f; //G
	gClearColor[2] = 0.25f; //B
...
{
	void render_sphere();
	void set_light_rotation(char axis_of_rotation, LIGHT_PROP& constantBufferLightProp);
	void set_material_properties(material_props_t material_properties, LIGHT_PROP& constantBufferLightProp);
	void draw_spheres(GLint no_of_spheres, material_props_t material_properties[], int no_of_materials, LIGHT_PROP& constantBufferLightProp);
	...
	set_light_rotation(current_axis_rotation, constantBufferLightProp);
	...
	draw_spheres(24, materials, no_of_materials, constantBufferLightProp); //Refer Material_prop.h for materials array details	
...
void set_material_properties(material_props_t material_properties, LIGHT_PROP& constantBufferLightProp) {
	...
	constantBufferLightProp.ka = XMVectorSet(material_properties.ambient[0], material_properties.ambient[1],
											material_properties.ambient[2], 1.0f);
	constantBufferLightProp.kd = XMVectorSet(material_properties.diffuse[0], material_properties.diffuse[1], 
											material_properties.diffuse[2], 1.0f);
	constantBufferLightProp.ks = XMVectorSet(material_properties.specular[0], material_properties.specular[1], 
												material_properties.specular[2], 1.0f);
	constantBufferLightProp.materialShininess = material_properties.shininess;	
...
void set_light_rotation(char axis_of_rotation, LIGHT_PROP& constantBufferLightProp) {
	...
	if (axis_of_rotation == 'x') {
		constantBufferLightProp.position = XMVectorSet(0.0f, RADIUS * (float)cos(gAngle), RADIUS * (float)sin(gAngle), 1.0f); //Note -ve Z for D3D
	}
	else if (axis_of_rotation == 'y') {
		constantBufferLightProp.position = XMVectorSet(RADIUS * (float)cos(gAngle), 0.0f, RADIUS * (float)sin(gAngle), 1.0f); //Note -ve Z for D3D		
	}
	else if (axis_of_rotation == 'z') {
		constantBufferLightProp.position = XMVectorSet(RADIUS * (float)cos(gAngle), RADIUS * (float)sin(gAngle), 0.0f, 1.0f); //Note -ve Z for D3D		
	}
}
void draw_spheres(int no_of_spheres, material_props_t material_properties[], int no_of_materials, LIGHT_PROP& constantBufferLightProp) {
	...
	int viewPortSizeX = winWidth / 8;
	int viewPortSizeY = winHeight / 8;
	GLfloat newAspectRatio = (float)viewPortSizeX / (float)viewPortSizeY;
	//update perspective according to new aspect ratio
	gPerspectiveProjectionMatrix = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0f), newAspectRatio, 0.1f, 100.0f);

	//glViewport(windowCenterX, windowCenterY, viewPortSizeX, viewPortSizeY);
	// Like glViewPort, set D3D view port
	D3D11_VIEWPORT d3dViewPort;
	d3dViewPort.TopLeftX = windowCenterX;
	d3dViewPort.TopLeftY = windowCenterY;
	d3dViewPort.Width = (float)viewPortSizeX;
	d3dViewPort.Height = (float)viewPortSizeY;
	d3dViewPort.MinDepth = 0.0f;
	d3dViewPort.MaxDepth = 1.0f;
	...
	int currentViewPortX = windowCenterX - x_trans * 2.5;
	int currentViewPortY = windowCenterY + y_trans*1.5;
	d3dViewPort.TopLeftX = currentViewPortX;
	d3dViewPort.TopLeftY = currentViewPortY;
	d3dViewPort.Width = (float)viewPortSizeX;
	d3dViewPort.Height = (float)viewPortSizeY;
	gpID3D11DeviceContext->RSSetViewports(1, &d3dViewPort);
	...
	for (i = 1, j = 0;i <= no_of_spheres;i++, j++) {
		...
		int local_y_trans = 0;
		if (((i - 1) % 4) == 0 && (i - 1 != 0)) {
			currentViewPortX = windowCenterX - x_trans * 2.5;
		...
		if (j <no_of_materials)
			set_material_properties(material_properties[j], constantBufferLightProp);
