...
HRESULT initialize()
{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceVertices, mappedSubresourceColors;
	ZeroMemory(&mappedSubresourceVertices, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer, NULL, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresourceVertices);
	memcpy(mappedSubresourceVertices.pData, vertices, sizeof(vertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer, NULL);

	//Copy color info into the color buffer
	ZeroMemory(&mappedSubresourceColors, sizeof(D3D11_MAPPED_SUBRESOURCE));

	gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresourceColors);
	memcpy(mappedSubresourceColors.pData, triangleColors, sizeof(triangleColors));
