...
ID3D11Buffer *gpID3D11Buffer_ConstantColorBuffer = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBufferWvp = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("D3D11-Geometry shader-Multicolored triangle");
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		"};"\
		"vertex_output main(float4 pos: POSITION) : SV_POSITION"\
		"{"\
	...
	*/
	...
	hr = D3DCompile(geometryShaderSourceCode, lstrlenA(geometryShaderSourceCode) + 1,
		...
		0,
		0,
		&pID3DBlob_geometryShaderSourceCode,
		&pID3DBlob_Error);
	if (FAILED(hr))
	{
		if (pID3DBlob_Error != NULL)
	...
	hr = gpID3D11Device->CreateGeometryShader(pID3DBlob_geometryShaderSourceCode->GetBufferPointer(),
		pID3DBlob_geometryShaderSourceCode->GetBufferSize(), NULL, &gpID3D11GeometryShader);
	if (FAILED(hr))
	{

		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() Failed for Geometry shader\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() succeeded for Geometry shader: \n");
		fclose(gpFile);
	}
	...
	//Define and set constant buffer for color to GS
	ZeroMemory(&bufferDesc_ConstantBuffer, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBuffer.Usage = D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBuffer.ByteWidth = sizeof(CBUFFER_Color);
	bufferDesc_ConstantBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, nullptr, &gpID3D11Buffer_ConstantColorBuffer);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed for constant color buffer.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() succeeded for constant color buffer.\n");
		fclose(gpFile);
	}
	//Define and set constant buffer
	D3D11_BUFFER_DESC bufferDesc_ConstantBufferWvp;
	ZeroMemory(&bufferDesc_ConstantBufferWvp, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBufferWvp.Usage = D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBufferWvp.ByteWidth = sizeof(CBUFFER);
	bufferDesc_ConstantBufferWvp.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBufferWvp, nullptr, &gpID3D11Buffer_ConstantBufferWvp);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed for constant buffer.wvp\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() succeeded for constant buffer wvp.\n");
		fclose(gpFile);
	}
	gpID3D11DeviceContext->VSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer);
	gpID3D11DeviceContext->GSSetConstantBuffers(0, 2, &gpID3D11Buffer_ConstantBufferWvp);
	gpID3D11DeviceContext->GSSetConstantBuffers(1, 2, &gpID3D11Buffer_ConstantColorBuffer);
...
void display()
{
	...
	worldMatrix = XMMatrixTranslation(0.0f, 0.0, 3.0f);
	...
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBufferWvp, 0, NULL, &constantBuffer, 0, 0);
	CBUFFER_Color colorsConstBuff;
	colorsConstBuff.color[0] = XMVectorSet(1.0, 0.0, 0.0, 0.0);//RED
	colorsConstBuff.color[1] = XMVectorSet(0.0, 1.0, 0.0, 0.0);//GREEN
	colorsConstBuff.color[2] = XMVectorSet(0.0, 0.0, 1.0, 0.0);//BLUE
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantColorBuffer, 0, NULL, &colorsConstBuff, 0, 0);
