...
HRESULT initialize()
{
	...
	float cubeData[] =
	{
		// SIDE 1 ( TOP )
		// triangle 1
		//first 3 are positions then 2 are tex coords
	...
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(cubeData);
	...
	inputElementDesc[0].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;
	...
	inputElementDesc[1].AlignedByteOffset = sizeof(float)*3;
...
void display()
{
	...
	UINT stride[] = { sizeof(float) * 5, sizeof(float)*5 };
	UINT offset[] = { 0, 0 };

	//******Quad drawing using TRIANGLE LIST **********************
	//Pass vertices data
	gpID3D11DeviceContext->IASetVertexBuffers(0, 2, &gpID3D11Buffer_QuadVertexBuffer, stride, offset);
	//Tex coords. Note input slot
	gpID3D11DeviceContext->IASetVertexBuffers(1, 2, &gpID3D11Buffer_QuadVertexBuffer, stride, offset);
	...
	worldMatrix = XMMatrixTranslation(0.0f, 0.0, 6.0f); ////Shift triangle to slightly right
