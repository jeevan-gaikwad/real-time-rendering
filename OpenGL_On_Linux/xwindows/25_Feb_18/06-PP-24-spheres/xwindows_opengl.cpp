...
int main(void){
	...
	cout<<"Initial window width:"<<winWidth <<" height:"<< winHeight<<endl;
	char ascii_val[32];
	CreateWindow();
	bool bDone=false;
	  ...
	  while(XPending(gpDisplay)){
		...
		switch(event.type){
			...
			case ConfigureNotify:
					...
					cout<<"window widhth:"<<giWindowWidth <<" window height:" <<giWindowHeight<<endl;
...
void CreateWindow(){
	...
	XStoreName(gpDisplay,gWindow,"PP-24-materials");
...
void draw_spheres(GLint no_of_spheres, material_props_t material_properties[], int no_of_materials) {
	...
	//winWidth = 800;
	//winHeight = 600;
	GLint windowCenterX = giWindowWidth / 2;
	GLint windowCenterY = giWindowHeight / 2;
	GLint viewPortSizeX = giWindowWidth / 8;
	GLint viewPortSizeY = giWindowHeight/8;
